...
@Disabled//(name = "OdometryTest [NON-WORKING???]", group = "odotest")
public class thesecondodotest extends OpMode {
    ...
    double x1position = leftFrontDrive.getCurrentPosition();
    double x2position = rightFrontDrive.getCurrentPosition();
    double yposition = leftBackDrive.getCurrentPosition();
    ...
    public void loop() {
        ...
        if (gamepad1.left_bumper || gamepad1.right_bumper) {
            power = 1; //divides power output by 1, so full power is used
        } else {
            power = 2.5; //divides power output by 2.5, so the robot's speed is halved to an easily controllable pace
        }

        // Tech Titans Standard Mechanum Drivetrain Control Layout (TTSMDCL) for gamepad 1
        rightFrontDrive.setPower((gamepad1.left_stick_x + gamepad1.right_stick_y + gamepad1.right_stick_x) / power);
        rightBackDrive.setPower((gamepad1.left_stick_x + (gamepad1.right_stick_y - gamepad1.right_stick_x)) / power);
        leftFrontDrive.setPower((-gamepad1.left_stick_x + (gamepad1.right_stick_y - gamepad1.right_stick_x)) / power);
        leftBackDrive.setPower((-gamepad1.left_stick_x + gamepad1.right_stick_y + gamepad1.right_stick_x) / power);
